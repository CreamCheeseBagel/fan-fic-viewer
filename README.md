...
headers, the request is routed through a public **CORS proxy** (selectable in
settings).

Two consequences to be aware of:

1. **Proxies are unreliable.** Public CORS proxies get rate-limited or go down.
   If a load fails, open ⚙ settings and pick a different proxy.
2. **Bot protection.** fanfiction.net sits behind Cloudflare, which sometimes
   serves a challenge page instead of the story. When that happens you'll get a
   "block/challenge page" error — try another proxy, or try again later.
